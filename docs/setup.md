...
- Which dashboard access method do they want?
  ...
  2. Cloudflare custom subdomain (planned, not automated in Alpha 16).
...
A domain is optional. Keep the dashboard bound to server loopback. Use Tailscale for routine private phone access or an SSH tunnel for temporary desktop access. If the owner selects Cloudflare, stop before generating the Alpha 16 plan and explain that its adapter is not implemented yet.
...
~~~json
{
  ...
  "fleet": {
    "assistants": [
      ...
      Cloudflare and immediate private Git setup remain planned options. Do not place them in an Alpha 16 setup plan.

      Alpha 16 delivers update notices through Telegram. Set `daily_check` to `false` when Telegram is not selected.
      ...
      git clone --branch v0.1.0-alpha.16 --depth 1 https://github.com/wetek/aidee.git
cd aidee
