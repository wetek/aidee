...
- Which dashboard access method do they want?
  ...
  2. Cloudflare custom subdomain (planned, not automated in Alpha 20).
...
A domain is optional. Keep the dashboard bound to server loopback. Use Tailscale for routine private phone access or an SSH tunnel for temporary desktop access. If the owner selects Cloudflare, stop before generating the Alpha 20 plan and explain that its adapter is not implemented yet. They may still record a custom HTTPS origin later. The controller writes that origin to the fleet registry and the assistant `config.yaml`. Telegram menu buttons must copy that URL.
...
~~~json
{
  ...
  "fleet": {
    "assistants": [
      ...
      Cloudflare and immediate private Git setup remain planned options. Do not place them in an Alpha 20 setup plan.

      Alpha 20 offers Start update on Telegram. Set `daily_check` to `false` when Telegram is not selected.
      ...
      git clone --branch v0.1.0-alpha.20 --depth 1 https://github.com/wetek/aidee.git
cd aidee
