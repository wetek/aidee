...
TOOL = ROOT / "platform" / "controller-tools" / "install-health-watchdog.py"


class HealthWatchdogTests(unittest.TestCase):
    def test_creates_one_approved_health_watchdog_job(self):
...
if [[ "$1" == "cron" && "$2" == "create" ]]; then
  printf '%s\n' "$*" >> "$AIDEE_TEST_CALLS"
  printf '%s\n' "Aidee fleet health watchdog" >> "$AIDEE_TEST_JOBS"
  exit 0
fi
exit 1
"""
            ...
            self.assertIn("[SILENT]", create_calls)
