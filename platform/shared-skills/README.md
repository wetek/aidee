...
`controller-update/SKILL.md` notices a newer Aidee tag on Telegram, then previews and applies the fleet updater after the owner sends Start update and Apply now. Cron notices use `send-telegram-choices.py` because Hermes cron cannot use clarify.
