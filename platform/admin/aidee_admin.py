    ...
    if assistant.get("kind") in {"coding", "project"}:
        sections.extend(
            [
                ...
                "- Test-driven verification (`tdd`): enforce TDD (strict Red-Green-Refactor loop) and execute real tests (tsc, pytest, vitest) before completing tasks. Never finish without test evidence.",
                "- Systematic debugging (`diagnose`): follow 6-phase root-cause analysis (Reproduce -> Minimise -> Hypothesise -> Instrument -> Fix -> Regression-test) before modifying code.",
                "- Requirements interrogation (`grill-me`, `to-prd`): interrogate requirements and edge cases before coding (`grill-me`), and synthesize clear specifications into actionable PRDs with acceptance criteria (`to-prd`).",
                "- Modularity & refactoring (`improve-architecture`): reduce accidental complexity, decouple modules, and fight code entropy without breaking tests.",
